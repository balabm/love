# --- IntelligenceDashboard.jsx ---
dash_path = "ui/src/components/IntelligenceDashboard.jsx"
with open(dash_path, "r", encoding="utf-8") as f:
    dash = f.read()

old_end = '    { name: "regret_minimizer", stats: modernStats.evolution_health?.regret_minimizer },\n  ].filter(m => m.stats);'
new_end = '''    { name: "regret_minimizer", stats: modernStats.evolution_health?.regret_minimizer },
    { name: "cognitive_bias_detector", stats: modernStats.evolution_health?.cognitive_bias_detector },
    { name: "mental_model_trainer", stats: modernStats.evolution_health?.mental_model_trainer },
    { name: "first_principles_thinker", stats: modernStats.evolution_health?.first_principles_thinker },
    { name: "systems_thinking_coach", stats: modernStats.evolution_health?.systems_thinking_coach },
  ].filter(m => m.stats);'''

if old_end in dash:
    dash = dash.replace(old_end, new_end)
    print("IntelligenceDashboard.jsx updated.")
else:
    print("WARNING: Could not find insertion point in IntelligenceDashboard.jsx")

with open(dash_path, "w", encoding="utf-8") as f:
    f.write(dash)

# --- SentinelPanel.jsx ---
panel_path = "ui/src/components/SentinelPanel.jsx"
with open(panel_path, "r", encoding="utf-8") as f:
    panel = f.read()

old_marker = '"regret_minimizer",'
new_marker = '"regret_minimizer", "cognitive_bias_detector", "mental_model_trainer", "first_principles_thinker", "systems_thinking_coach",'

count = panel.count(old_marker)
if count == 3:
    panel = panel.replace(old_marker, new_marker)
    print(f"SentinelPanel.jsx updated ({count} occurrences).")
else:
    print(f"WARNING: Found {count} occurrences of marker, expected 3.")

with open(panel_path, "w", encoding="utf-8") as f:
    f.write(panel)

print("UI Batch 51 update complete.")
